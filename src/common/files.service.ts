...
import sharp from 'sharp'; // Comentario: Asegúrate de tener instalado @types/sharp [cite: 2026-02-25]

@Injectable()
export class FilesService {
  // Comentario: Definimos la ruta raíz de subidas fuera del método para reusarla [cite: 2026-02-23]
  ...
  async saveAndOptimize(file: Express.Multer.File, folder: 'bases' | 'designs') {
    ...
    // Comentario: Asegurar que existen las carpetas necesarias [cite: 2026-02-23]
    [targetFolder, thumbFolder].forEach(dir => {
      if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
    });

    // Comentario: Generamos un nombre único para evitar colisiones [cite: 2026-02-23]
    ...
    try {
      ...
      // Comentario: Escribimos el buffer del archivo en la carpeta destino [cite: 2026-02-23]
      ...
      // Comentario: Convertimos a WebP para reducir peso en el frontend [cite: 2026-02-25]
      ...
      // Comentario: Retornamos rutas relativas para guardar en la base de datos [cite: 2026-02-23]
      return {
